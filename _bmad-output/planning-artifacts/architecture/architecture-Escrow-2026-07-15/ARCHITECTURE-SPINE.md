...
- **Rule:** au `flush()`, classer chaque échec de rejeu **par le code applicatif de l'enveloppe d'erreur, jamais par la seule classe HTTP** (un `409` est ambigu). **Transitoire** (⇒ conserver l'entrée + binaire, re-tenter) : offline / réseau, `5xx`, `408`, `429`, et **`409` optimistic-lock** (`@Version`). **Permanent** (⇒ **annuler l'affichage optimiste**, **conserver** l'entrée + binaire, **notifier** avec motif + état réel, **stopper l'auto-retry**) : un ensemble **énuméré** de codes applicatifs (ex. `DISPUTE_ALREADY_RESOLVED`, `EVIDENCE_INVALID`, `WINDOW_CLOSED`). **Aucun fichier n'est perdu silencieusement.** *(Amendé 2026-07-17 — escalation 4.3 : ce classement suppose un champ **`code`** dans l'enveloppe d'erreur, qui n'existait pas. Il est livré par la **Story 5.3** (prérequis backend) ; l'Epic 4 n'est donc **pas** purement frontend, contrairement à la note de découpage initiale. La `Reason` machine-readable de `TransitionException`, jusqu'ici jetée, est généralisée à toutes les exceptions métier.)*
