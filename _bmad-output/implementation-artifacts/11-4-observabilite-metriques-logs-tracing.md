...
- [x] **T5 — Alerting : canal et règles minimales** (AC: 3) — *2026-08-14*
  - [x] Canal selon Q2 : Alertmanager → SMTP → Mailpit. Ce qu'il **n'est pas** est écrit en tête d'`alertmanager.yml` : ni canal de production, ni décision du spike AR-P6. Le jour où AR-P6 tranche, c'est la valeur de `smtp_smarthost` qui change, pas l'architecture.
  - [x] **DÉCISION D'OUVERTURE — la DLQ n'est PAS créée ici.** Déclarer une file de rebut change le TRAITEMENT des messages en échec : ce n'est pas de l'observabilité (même raisonnement que « aucun bump de Spring Boot », Q4). La règle est donc écrite, sa logique prouvée par `promtool` sur série synthétique, et **son inertie écrite dans le fichier** plutôt que déduite. Son sélecteur `.+\.dlq` est posé comme **contrat de nommage** : une DLQ future nommée autrement ne serait jamais vue, en silence — le mode de défaut est nommé d'avance.
  - [x] **Le critique ne se groupe pas** : route dédiée `severity=critical` avec `group_wait: 0s`. Le défaut de 30 s aurait retenu l'alerte que l'AC4 exige « immédiatement ».
  - [x] Les trois règles minimales, chacune assise sur une série vérifiée présente en T4 : `HighServerErrorRate` (5xx), `DeadLetterQueueNotEmpty` (inerte, voir ci-dessus), `BrokerResourceAlarm` + `ConnectionPoolSaturated` (saturation disque **et** pool — l'AC en nomme deux, elles ont deux sources et donc deux règles).
  - [x] **Harnais `promtool test rules`, puisque `mvn test` ne voit rien de tout cela.** 10 cas, chaque règle éprouvée en positif **et** en négatif : un cas « ça se déclenche » seul est satisfait par une règle qui se déclenche toujours. Trois mutations passées : `for: 5m` retiré → 1 rouge ; sélecteur DLQ neutralisé → 1 rouge ; chaîne `or` des alarmes amputée → **2 rouges**, ceux des deux alarmes retirées, ce qui prouve qu'elles sont bien testées séparément.
  - [x] ⚠️ **Un cas de test s'est trompé de cible avant d'être corrigé** : le contrôle du `for: 5m` évaluait à 7 min, où l'alerte est déjà partie légitimement. L'instant d'évaluation est load-bearing ; à 4 min la garde est observable, et elle rougit sous mutation.
  ...
  - [x] **Preuve de bout en bout consignée.** Règle temporaire `AlertingChannelSmokeTest` (`vector(1)`, sévérité critique) chargée, puis suivie à chaque étape : Prometheus `état=firing` → Alertmanager `statut=active receveurs=['exploitant']` → **message reçu dans Mailpit**, objet `[FIRING × 1] AlertingChannelSmokeTest`, de `alertes@escrow.local` à `exploitant@escrow.local`, corps portant bien les libellés français. Règle retirée ensuite : **le message `[RESOLVED]` est arrivé lui aussi**, ce qui prouve au passage `send_resolved` — sans quoi l'exploitant saurait quand ça casse et jamais quand c'est réparé.
