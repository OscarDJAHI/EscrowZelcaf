...
- **Classement des échecs (AD-10), le cœur de l'epic.** Au `flush()`, classer chaque échec **par le code applicatif de l'enveloppe d'erreur, jamais par la seule classe HTTP** — un `409` est ambigu. **Transitoire** (conserver + re-tenter) : offline/réseau, `5xx`, `408`, `429`, `CONCURRENT_MODIFICATION` (collision de verrou optimiste, rendue en `409`), `FILE_READ_ERROR`, timeout/rate-limit. **Permanent** (annuler l'optimiste, **conserver** l'entrée + binaire, notifier avec motif + état réel, stopper l'auto-retry) : `DISPUTE_ALREADY_RESOLVED`, `TRANSACTION_TERMINAL`, `WINDOW_CLOSED`, `EVIDENCE_INVALID`, `NOT_A_PARTY`, `TRANSACTION_NOT_FOUND`, `EVIDENCE_FLOOR_VIOLATION`, `COMMENT_TOO_SHORT`, `TOO_MANY_FILES`. Cette classification remplace le `break` aveugle actuel du store.
- **Enveloppe d'erreur — amendement AD-10 (2026-07-17).** Le classement par code supposait un champ **`code`** que l'enveloppe (`{timestamp,status,error,message}`) ne portait pas : l'escalation de la Story 4.3 a rouvert le point. Le champ `code` — identifiant machine stable en SCREAMING_SNAKE_CASE, distinct de la reason-phrase HTTP — est livré par la **Story 5.3** (prérequis backend bloquant, à consommer, pas à re-livrer), qui généralise et sérialise la `Reason` machine-readable jusqu'ici jetée et mappe la collision de verrou optimiste sur `409 CONCURRENT_MODIFICATION` au lieu d'un `500`. **Conséquence de cadrage : l'Epic 4 n'est pas purement front-end**, contrairement à la note de découpage initiale ; l'énumération de codes ci-dessus fait foi et est partagée avec le backend. Ne jamais matcher le texte du `message` : il est instable. Si un code permanent nécessaire manque à l'appel, c'est un conflit à remonter — pas une raison de retomber sur la classe HTTP.
...
- **Périmètre de code.** Travail concentré sur `frontend/src/stores/offlineQueue.js` (refonte), le store `escrow` (état optimiste), `api/evidence.js` et les composants de dépôt/liste. Stack front existante : Vue 3 + Pinia + Vite/PWA + Tailwind. Le seul appui backend est le contrat de codes de la Story 5.3 ; les endpoints consommés (composite, dépôt, liste, download) sont déjà livrés.
...
- **Dépend de la Story 5.3** (contrat d'erreur codé) pour les Stories 4.3/4.4/4.5 : sans le champ `code` et son énumération, la classification transitoire/permanent d'AD-10 n'est ni fiable ni testable. Prérequis backend bloquant, issu de l'escalation 4.3.
