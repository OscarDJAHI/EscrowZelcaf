...
- Gardes centralisées AVANT toute opération : appartenance transaction (`TransactionAccess`, AD-3), gating KYB/suspension (AD-20), rôle interne MANAGER/MEMBER (AD-30). Ne jamais recoder un contrôle par endpoint.
- **Anti-énumération (NFR-P9, Story 1.10) — convention transverse, à réutiliser telle quelle par toute nouvelle ressource protégée** :
  - Règle : ne jamais révéler l'existence d'une ressource que l'appelant **n'est pas autorisé à connaître**. « Inexistante » et « appartenant à un tiers » répondent la **même chose** : `404` + même `code` + même `message` + mêmes en-têtes. Le `code` est un oracle au même titre que le statut (AD-10) : uniformiser l'un sans l'autre ne ferme rien.
  - Comment : **une seule fabrique par famille de ressource** dans `web/ApiExceptions` (`transactionNotFound()`, `evidenceNotFound()` ; ajouter la sienne pour wallet, ticket, fil de litige, dossier KYB), jetée depuis la garde centralisée — jamais deux littéraux à deux endroits, ils divergent en silence. **Aucun identifiant dans le message** (il est déjà dans l'URL) : c'est ce qui rend les deux réponses comparables littéralement.
  - Interdit : créer un `code` qui nomme le refus d'appartenance (`NOT_A_PARTY` a été **supprimé** de `ErrorCode`, et `ErrorCodeContractTest.membershipRefusalHasNoDedicatedCode` interdit d'en réintroduire un sous un autre nom).
  - **Exception documentée** : un refus honnête reste permis quand l'appelant **est déjà autorisé à connaître** la ressource (ex. une partie qui tente de retirer la pièce de l'autre : `403 / FORBIDDEN`, car elle voit déjà cette pièce via `GET /{id}/evidence`). Toute exception de ce type doit être commentée **et** prouvée par un test jumeau qui démontre la visibilité préalable.
  - Preuve attendue : comparer les **deux réponses entières** (statut + corps + en-têtes, horodatage normalisé) au niveau HTTP, pas le seul type d'exception au niveau service — gabarit : `AntiEnumerationIntegrationTest`. Vaut aussi pour le canal partenaire HMAC, pas seulement pour le canal JWT.
- Ne jamais toucher au `403` **nu** de Spring Security (`Http403ForbiddenEntryPoint`, sans enveloppe ni `code`) : la Story 1.9 en a fait le discriminant « session expirée » côté client. Un 403 nu déconnecte, un 4xx **codé** ne déconnecte pas.
...
- Backend : 481 tests, `./mvnw test` — **Docker requis** (Testcontainers : Postgres réel + MinIO réels). H2 réservé aux slices rapides (state machine). Les preuves de concurrence (nonce, retrait, solde) se font sur **base réelle**, pas sur mocks.
- Tests dans le même paquet `com.zlecaf.escrow`, suffixe `*Test.java` ; intégration = `*IntegrationTest.java`.
- **Conteneur Postgres : NE JAMAIS en déclarer un nouveau.** Une suite qui a besoin de Postgres appelle `PostgresTestSupport.registerDatabase(registry, MaSuite.class)` depuis son `@DynamicPropertySource`, et n'écrit ni `@Container`, ni `@Testcontainers`, ni le nom d'image. Un conteneur unique sert toute la JVM ; chaque classe reçoit sa **propre base**, donc l'isolation d'aujourd'hui est préservée — ne pas « simplifier » en partageant une base commune, les suites `@SpringBootTest` committent pour de vrai et deviendraient dépendantes de leur ordre. La commande du conteneur (`fsync=off`, `max_connections=300`) est relue et asservie par `PostgresTestSupportTest` : `withCommand` écrase la commande posée par le CONSTRUCTEUR de `PostgreSQLContainer`, piège dans lequel la migration est tombée une fois — ne pas modifier cette commande sans regarder ce test.
- Frontend : 241 tests Vitest (`npm run test`), env jsdom, `testTimeout: 30000` (Blobs multi-Mo dans le structured clone), tests colocalisés en `__tests__/`.
- **Tests de composant : le harnais existe, s'en servir.** `vitest.config.js` porte le plugin `vue()` et `@vue/test-utils` est installé — `mount()` fonctionne. Gabarit : `src/components/__tests__/SyncFailureNotice.spec.js` (monte le composant réel, mocke le module d'API, assied ses assertions sur l'état du store et non sur des chaînes de gabarit). Couverture actuelle : 3 composants sur 14 — toute story qui livre ou réécrit un composant apporte son test de composant.
- **Lint frontend : `npm run lint` (`eslint . --max-warnings 0`), gate CI.** Jeu de règles `vue/flat/essential` (correction) + `js/recommended` + `no-await-in-loop`. `reportUnusedDisableDirectives: 'error'` — une directive `eslint-disable` qui ne supprime plus rien FAIT ÉCHOUER le lint. Ne pas passer à `flat/recommended` sans traiter les ~134 avertissements de formatage qu'il ajoute ; le style se décide avec un formateur, sur du code neuf.
- **Garde d'encodage : `python3 scripts/check-encoding.py`, gate CI.** Aucun fichier suivi ne porte d'octet NUL ni d'octet UTF-8 invalide. Corollaire pratique : **écrire le français avec ses accents**, y compris dans les commentaires de test — le contournement « sans accents » des Stories 1.9/1.10 n'a plus lieu d'être, la barrière prouve la validité UTF-8 à chaque push. Un en-tête magique (GIF, PNG…) s'écrit en tableau d'octets explicite, jamais en littéral à octets bruts : c'est le défaut qui a rendu `AuthView.vue` puis `EvidenceServiceTest.java` binaires pour git, donc invisibles à `grep` — donc improuvables.
...
- **Vérification par mutation, obligatoire sur tout correctif de sécurité** (rétrospective Epic 1, 2026-07-27). Un correctif qui prétend fermer un trou doit être prouvé en **retirant la garde et en constatant le rouge**, puis en la restaurant. Consigner le résultat dans la spec.
  - _Pourquoi cette règle existe :_ trois fois dans l'Epic 1, un correctif validé et « testé » ne prouvait **rien**. Le `CHECK` de la 1.7 était plus large que ce que l'application reconnaît (un secret de 25 octets franchissait la garde restaurée pour couvrir ce chemin précis) ; la purge locale de la 1.9 n'a été déplacée qu'à moitié devant le réseau **deux passes de suite**, parce que le test ne contrôlait que la moitié déjà déplacée ; le test de convention de la 1.10 ratait les noms qualifiés et les appels multilignes, laissant l'oracle rouvrable au vert. Dans les trois cas le **code** était bon et la **preuve** était creuse — un test qui passe autant avec le correctif que sans lui ne dit rien de la propriété qu'il nomme.
  - _Portée :_ tout correctif touchant une garde d'autorisation, d'appartenance, de cloisonnement inter-sociétés, d'anti-énumération, de limitation de débit, de chiffrement ou de validation d'entrée. Hors périmètre : le cosmétique, la documentation, le renommage.
  - _Forme :_ mutation la plus proche possible du défaut d'origine (pas une mutation commode), suite relancée, **et** vérification que ce sont bien les tests visés qui rougissent. Gabarits dans `WebhookControllerTenantScopeTest` (`verify(repo, never()).findAll()` — ce qui interdit le chemin non borné, pas seulement le résultat filtré) et `GlobalExceptionHandlerTest.hostileAcceptLeavesEveryBranchCoded`.
